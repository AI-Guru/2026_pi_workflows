...
3. **File into the library** — once title/author are known, move the source file to `books/Letter<X>/<Title, ':'→'_'> - <Author>.<ext>` (X = the title's first letter), unless `--no-organize` is passed. All derived outputs are written next to wherever the book ends up.
